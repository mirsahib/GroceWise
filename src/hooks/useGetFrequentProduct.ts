...
const useGetFrequentProduct = () => {
    ...
    const getProductData = async () => {
        try {
            ...
            const frequentProduct = findMostFrequentProductIds(userProductJson.valueOf() as UserProductEntry[])
            console.log("🚀 ~ file: useGetFrequentProduct.ts:40 ~ getProductData ~ frequentProduct:", frequentProduct)
            ...
            if (frequentProduct.length !== 0) {
                ...
                console.log("🚀 ~ file: useGetFrequentProduct.ts:47 ~ getProductData ~ Products:", Products)
                setProduct(Products);
                setLoading(false);
            }
        } catch (error) {
